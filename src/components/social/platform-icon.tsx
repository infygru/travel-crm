...
const BG_COLORS: Record<SocialPlatform, string> = {
  FACEBOOK: "bg-blue-50",
...
export function PlatformIcon({ platform, size = 20, showBg = false, className = "" }: PlatformIconProps) {
  const Icon = ICONS[platform]
  const colorClass = COLORS[platform]
  const bgClass = BG_COLORS[platform]

  if (showBg) {
    return (
      <div className={`flex items-center justify-center rounded-lg p-1.5 ${bgClass} ${className}`}
